...
def compare_all_models_with_optuna(df, n_trials=30):
    """Tüm modelleri Optuna ile optimize et ve karşılaştır
    
    Modeller: Random Forest, SVM, Logistic Regression, Naive Bayes, XGBoost, KNN
    """
    print("\n" + "="*60)
    print("🔬 TÜM MODELLER KARŞILAŞTIRMASI (Optuna Optimizasyonu)")
    print("   Modeller: RF, SVM, LR, Naive Bayes, XGBoost, KNN")
    print("="*60)
    ...
    # =========================================
    print("\n📌 1/6: Logistic Regression optimizasyonu...")
    ...
    # 2. Random Forest
    # =========================================
    print("\n📌 2/6: Random Forest optimizasyonu...")
    ...
    study_nb.optimize(nb_objective, n_trials=n_trials, show_progress_bar=True)
    
    best_params_nb = study_nb.best_params
    best_nb = GaussianNB(**best_params_nb)
    model_results['Naive Bayes'] = evaluate_model(best_nb, X_resampled, y_resampled)
    best_params_all['Naive Bayes'] = best_params_nb
    print(f"   ✓ NB: F1={model_results['Naive Bayes']['f1']}")
    
    # =========================================
    # 5. XGBoost
    # =========================================
    print("\n📌 5/6: XGBoost optimizasyonu...")
    ...
    study_knn.optimize(knn_objective, n_trials=n_trials, show_progress_bar=True)
    
    best_params_knn = study_knn.best_params
    best_knn = KNeighborsClassifier(**best_params_knn, n_jobs=-1)
    model_results['KNN'] = evaluate_model(best_knn, X_resampled, y_resampled)
    best_params_all['KNN'] = best_params_knn
    print(f"   ✓ KNN: F1={model_results['KNN']['f1']}")
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("TÜM MODELLER - EN İYİ PARAMETRELER\n")
        f.write("=" * 60 + "\n")
    ...
    return model_results, best_params_all, X_resampled, y_resampled, features
...
def plot_model_comparison(model_results):
    ...
    # Renk paleti (6 model için)
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD']
    
    fig, axes = plt.subplots(1, 2, figsize=(16, 7))
    
    # F1 Score Bar Chart
    bars = axes[0].barh(models, f1_scores, color=colors[:len(models)], edgecolor='black')
    ...
    # Tüm metrikler
    metrics = ['acc_mean', 'recall_mean', 'auc_mean']
...
def generate_detailed_visualizations(X_resampled, y_resampled, features, best_params=None):
    ...
    plot_confusion_matrix(y_resampled, y_pred, "Confusion Matrix (En İyi Model)")
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, color='#4ECDC4', linewidth=2, label=f'Random Forest (AUC = {roc_auc:.3f})')
    ...
    plot_feature_importance(model, features, "Özellik Önem Sıralaması (Random Forest)")
...
def main():
    ...
    # Tüm modeller karşılaştırması (Optuna ile) - Tek seferde
    model_results, best_params_all, X_resampled, y_resampled, features = compare_all_models_with_optuna(df, n_trials=30)
    
    # Detaylı görselleştirmeler (model karşılaştırmasından gelen verilerle)
    rf_params = best_params_all.get('Random Forest', None)
    generate_detailed_visualizations(X_resampled, y_resampled, features, rf_params)
    ...
    # En iyi model
    best_model = max(model_results.items(), key=lambda x: x[1]['f1_mean'])
    print(f"\n🏆 En İyi Model: {best_model[0]}")
